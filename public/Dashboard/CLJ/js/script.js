...
let attempts = quizData.attempts;
let highestScore = quizData.highestScore;
...
function showResult() {
    ...
    const updatedQuizData = {
        userScore: userScore,
        attempts: attempts,
        highestScore: quizData.highestScore // Update highest score
    };
...
function startTimer(time) {
    ...
    function timer() {
        ...
        if (time < 0) { //if timer is less than 0
            ...
            let correcAns = questions[que_count].answer; //getting correct answer from array
            for (let i = 0; i < allOptions; i++) {
                if (option_list.children[i].textContent == correcAns) { //if there is an option which is matched to an answer
                    option_list.children[i].setAttribute("class", "option correct"); //adding green color to matched option
                    option_list.children[i].insertAdjacentHTML("beforeend", tickIconTag); //adding tick icon to matched option
